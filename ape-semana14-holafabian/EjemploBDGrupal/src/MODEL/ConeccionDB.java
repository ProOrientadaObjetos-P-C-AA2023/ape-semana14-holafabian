...
public class ConeccionDB {
    ...
    public void eliminarEstudiante(String nombreEstudiante) {
        try {
            setConcDB("jdbc:sqlite:bd/dbTest1.db");
            Statement statement = concDB.createStatement();
            String strDeleteEst = String.format("DELETE FROM Estudiante WHERE nombreEst = '%s'", nombreEstudiante);
    ...
    public void actualizarEstudiante(Estudiante estudiante) {
        try {
            setConcDB("jdbc:sqlite:bd/dbTest1.db");
            Statement statement = concDB.createStatement();
            String strUpdateEst = String.format("UPDATE Estudiante SET nota1 = %f, nota2 = %f, promedio = %f, estado = '%s' WHERE nombreEst = '%s'",
                    estudiante.nota1, estudiante.nota2, estudiante.promedio, estudiante.estado, estudiante.nombreEst);
